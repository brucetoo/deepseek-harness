...
> 范围：完成的轮次以其产出文件收尾的那一行、会话级成果视图、对外部生成文件的显式登记、读得出是链接的文件路径链接，以及 Host 打开器对浏览器可渲染文档优先选用默认浏览器。经决定不在范围内：以 HTTP 提供工作区文件，以及为不在 Host 机器上的客户端提供预览。
...
**完成的一轮以它产出的文件收尾。** 该行是独立插件 `@deepseek-ai/dsh-client-ui-deliverables`，注册进 chat 视图在收尾消息正文与其 IconActions 之间渲染的 `conversation.chat.turnTail` 空位——ui-conversation 拥有空位与 owner 通货（节点、收尾 seq、`openFile`），插件拥有全部策略。`producedForClosing` 从改写工具自身的跟随文件 `locations` 中读出路径——diff 卡片，或 `kind` 为 `edit` 的 generic 卡片——因此无论模型是否记得点名，这一轮的产出都会被列出。后者包括 `str_replace_editor` 插入操作和 `register_artifact`；`register_artifact` 会先验证由其他工具或脚本创建的文件存在且为普通文件，再发布其位置。read、删除与失败的调用不贡献任何条目；同一路径在一轮内按首见顺序只出现一次；累积在 turn 边界重置，因此一轮若先修改文件、随后没有正文内容就结束，不会溢进下一轮的行里。单行 lane 会测量 chip 和本地化剩余计数，再显示能放下的最大前缀（至多六个）及 `+ N 个文件`。cordis.yml 中的一行即可把该交互面组合进来或去掉；未注册的空位什么也不渲染。

**同一批事实构成会话级成果视图。** 本插件注册一个 `conversation.view` 条目，折叠每个已加载 Turn 的成果数据，让每条路径只保留一行，并按最近一次成功修改或登记排序。既有 Session 分页器按需加载被省略的历史前缀。这样 registry 可从 Session 日志重建，无需第二种持久化格式或 wire API。
...
- **从终端输出推断文件，或在每条命令后扫描 workspace**——输出文本没有可靠的路径语法，而目录差异既昂贵，也会与无关写入竞争，且无法证明哪些文件是预期成果。`register_artifact` 让生产方显式声明一条经过验证的现有路径。
...
现有的每一处文件交互都同时改变了：write、edit、read、已登记成果与通用单文件卡片都汇到 `openFile`，因此链接修复与浏览器优先策略无需逐行改动。组装层 Web 测试覆盖溢出几何和单次点击的 Host 交接，且不会启动原生应用。会话视图只能显示已加载的历史，并在仍有更早前缀时暴露既有分页器。产出的 `file://` 文档无法 `fetch` 同级文件（但 `<script src>`、`<img>` 和 CSS `@import` 可用），这是 HTTP 提供曾有、而此处没有的能力。远程客户端保留 chip，但省略文件夹操作；每个 chip 的 `title` 仍保留完整路径。Markdown 仍由平台的 `.md` 应用打开；产品内渲染属于另一项工作。
