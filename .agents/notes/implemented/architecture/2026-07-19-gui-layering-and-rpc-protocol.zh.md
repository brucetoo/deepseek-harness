...
同时各消费方的物理通道不同（浏览器与 Electron HTTP／WebSocket、进程内 fetch/SSE、将来可选的 IPC），还需要一个通道无关的消息模型和单一约定真源，让「加一个方法」「换一种载体」互不牵连，且 wire 上的每条消息可类型校验、可观测、可对账。
...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop`（`@deepseek-ai/dsh-desktop`）把同一 Web 组合打包在加固的 Electron 窗口与独立 Node sidecar 后面，通过已鉴权的 loopback 复用 HTTP／WebSocket 载体（[桌面决策](2026-09-15-electron-desktop-application.zh.md)）。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器部署使用未鉴权的默认值；Electron 使用精确 origin 标头注入与可选 bearer 配置；零 workspace 依赖（注册表经结构注入到达） |
...
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web）+ `@deepseek-ai/dsh-desktop`（apps/desktop） | bin 粗分发、Vite 浏览器入口，以及 Electron 生命周期与打包入口 | 各应用保留私有的启动与打包知识；桌面应用把产品行为交给同一套 Host 与客户端插件 |
...
1. **选 fetch 伪造方式**：浏览器或 Electron 同源 HTTP / 进程内 `host.handler.fetch` 注入 / 自写传输切面子类（例如将来可选的 Electron IPC 载体，见下文「子类表」）。
2. **在 `apps/` 下写拼装模块**：`startHost()` + 客户端子类 + 该应用私有的信号/打印/退出语义；混合体不建包，拼装写在 app 里。
3. **需要 HTTP 承载才 import `dsh-host-webserver`**，否则零端口。

现有三个应用保持这一区分：Web 应用挂载 Host、载体与浏览器组合；桌面应用把该组合启动为已鉴权的 sidecar；`dsh --profile headless` 则挂载直接使用核心服务的 runner，不包含 Host、HTTP 或端口。ACP 类协议桥不遵循 client 载体清单：它把 core 暴露给外部生态，直接通过 `ctx.plugin(入口插件)` 挂载，不使用 fetch。
...
| `WebApiClient` | dsh-client-connection | `globalThis.fetch` 上行 + 每逻辑流一条同源 WebSocket 下行 | 浏览器与 Electron renderer 客户端；Electron 在本类下方注入 bearer 标头，物理边界见 [WebSocket 下行载体](2026-08-04-websocket-downlink-carrier.zh.md) |
