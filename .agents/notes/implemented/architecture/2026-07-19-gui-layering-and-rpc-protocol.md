...
At the same time the physical channels differ per consumer (browser and Electron HTTP/WebSocket, in-process fetch/SSE, optional IPC later), so we also need a channel-independent message model and a single contract source of truth — "adding a method" and "swapping a carrier" must not entangle each other, and every message on the wire must be type-validatable, observable, and reconcilable.
...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` (`@deepseek-ai/dsh-desktop`) packages the same Web composition behind a hardened Electron window and an independent Node sidecar. It reuses the HTTP/WebSocket carrier over authenticated loopback ([desktop decision](2026-09-15-electron-desktop-application.md)).

```
apps/*  (applications: web = vite, cli = bin dispatch, desktop = Electron)
  │ consume
  ▼
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser deployments use the unauthenticated default; Electron uses exact-origin header injection with the optional bearer config; zero workspace dependencies (the registry arrives by structural injection) |
...
| Application | `@deepseek-ai/dsh` (apps/cli) + `dsh-web-frontend` (apps/web) + `@deepseek-ai/dsh-desktop` (apps/desktop) | Coarse bin dispatch, the Vite browser entry, and the Electron lifecycle and packaging entry | Applications keep their private launch and packaging knowledge; the desktop app delegates product behavior to the same Host and client plugins |
...
1. **Pick a fetch impersonation**: browser or Electron same-origin HTTP / in-process `host.handler.fetch` injection / your own transport-aspect subclass (for example, an optional future Electron IPC carrier; see the "Subclass table" below).
...
The three applications preserve the division: the Web application mounts Host, carrier, and browser composition; the desktop application launches that composition as an authenticated sidecar; and `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
...
| `WebApiClient` | dsh-client-connection | `globalThis.fetch` uplink + one same-origin WebSocket downlink per logical stream | the browser and Electron renderer client; Electron injects its bearer header below this class, and the physical boundary is in the [WebSocket downlink carrier](2026-08-04-websocket-downlink-carrier.md) |
