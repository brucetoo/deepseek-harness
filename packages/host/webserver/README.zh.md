...
- **不提供 TLS 或来源策略**：bearer 鉴权为可选功能且不提供加密；绑定非回环地址仍会向对应网络暴露流量，因此部署需要 TLS 终止和自身的来源策略。
