...
合法请求进入活动 Provider 后，服务会发送一次 `user-question/requested` 事件，并携带已经校验的 `AskUserQuestionRequest`。该事件是用于通知和遥测的受控、非持久化观察 seam。监听器不能回答、取消或延迟请求；监听器抛出的同步或异步错误只会记录日志，不能改变 Provider 结果。进入 Provider 前被拒绝的请求不会产生该事件。
