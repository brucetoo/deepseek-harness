...
产出文件与可点击文件引用功能的属主。Node 侧注册最终回复文件引用指引；浏览器侧同时注册已完成轮次末尾的产出文件行与会话级**成果**视图，并将收尾正文中匹配的行内代码引用转换为链接。正式提供的 Web patch 加载本包。standard agent 的同级 [`dsh-tool-fs`](../../fs/tool-fs/) 包提供 `register_artifact`。
...
`DeliverablesView` 注册一个会话视图标签，并在已加载 timeline 上折叠同一份 Turn 数据。它让每条路径只出现一次，按最近一次成功修改排序，记录最近轮次，并通过同一个可感知 workspace 的 Host 回调打开文件。Session 尚有未加载的历史前缀时，**加载更早成果**会要求既有 Session runtime 获取下一页；系统不会建立另一套成果数据库或 wire endpoint。
...
`register_artifact` 会验证按 workspace 解析后的 `path` 指向现有普通文件，记录文件系统观察结果，并返回一个成功的 generic edit 展示，其中 `locations` 携带该路径。它不会创建、修改、读取、上传或复制文件。由此，生成二进制的工具或脚本可以发布持久成果事实，而客户端不必知道其工具名。文件系统工具包拥有其 schema、执行逻辑与提示词指引。

Node 侧会注册静态系统提示词段落 `ui:deliverable-file-references`。它要求模型点名主要输出，并使用已登记路径、文件工具路径或唯一 basename 将变更文件引用写成 Markdown 行内代码。该提示词只向模型说明渲染器接受的语法，不约束无关的路径讨论。
...
一段固定提示词要求模型在最终回复中点名主要文件，并把变更文件引用写成采用精确路径或唯一 basename 的 Markdown 行内代码，例如 `out/report.docx`。standard agent 还会从 `dsh-tool-fs` 单独获得 `register_artifact` schema 与指引。

#### Token 影响

加载本包时增加一段固定提示词；不增加按 Turn 变化的提示词上下文。

#### KV Cache 影响

该段落在本包加载期间保持静态，因此留在可复用的提示词前缀中，不会随 Turn 改变。
...
- **外部生成过程需要显式登记。**终端命令不会推断输出路径。文件存在后，agent 必须调用 `register_artifact`；遗漏或失败的登记不会进入成果与提及词表。
- **会话视图只覆盖已加载历史。**仍有更早页面时，它会显示**加载更早成果**，而不是另行执行全会话查询。
